...
#define num_data 5		// # of input data
#define sqr(x) 	((x)*(x))
...
int main() 
{
	if(EXCLUSIVE_ID==0)
	{
		...
		profiling_start("K-means clustering");
		///  k-means clustering  ///
		for(;;)
		{
			int flag = 0;
		...
		profiling_print();		// report total tick of the clustering loop (FPU vs non-FPU ?????
